...
export class StateTransitionSystem {
  ...
  constructor(
    ...
    this.description.transitions.forEach((transition) => {
      if (!this.transitionRules.get(transition.current))
        this.transitionRules.set(
          transition.current,
          new Map<string, Transition>()
        )
      if (!this.transitionRules.get(transition.next))
        this.transitionRules.set(transition.next, new Map<string, Transition>())
  ...
  init() {
    const state = this.createStateInstance(
      this.description.start,
      this.description.startLoadResources
    )
  ...
  private getNextInstance(
    event: GameEvent
  ): [string, GameState, boolean, boolean] {
    ...
    return [
      transition.next,
      next,
      transition.destroyCurrent,
      transition.resetEngine,
    ]
  }
